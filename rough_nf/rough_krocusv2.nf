...
process COMBINE_KROCUS_FILES {
    ...
    script:
    """
    KROCUS_FILES=(${krocus_individual_files})

    for index in \${!KROCUS_FILES[@]}; do
    KROCUS_FILE=\${KROCUS_FILES[\$index]}
    sample_id=\${KROCUS_FILE%.${allele_dir}.${kmer_size}mer.krocus.txt}

    awk -v OFS='\\t' '{print FILENAME, \$0}' \${KROCUS_FILE} | sed "s/\\.${allele_dir}\\.${kmer_size}mer\\.krocus\\.txt//g" |  grep -v 'ND' >> combined_krocus_${allele_dir}_${kmer_size}mer_${date}.txt
...
workflow  {
	  ...
	  collected_krocus_ch = KROCUS.out.indiv_krocus_ch.collect( sort: {a, b -> a[0].getBaseName() <=> b[0].getBaseName()} )
	 // collected_krocus_ch.view()
